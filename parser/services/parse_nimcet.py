...
def extract_candidate_details(text: str) -> Dict[str, str]:
    ...
    app_no_match = re.search(
        r"Application\s*(?:Seq\s*)?(?:No\.?|Number)\s*:?\s*([A-Z0-9]+)",
        text,
        re.IGNORECASE
    )
    if app_no_match:
    ...
    # ✅ FIXED Candidate Name Extraction
    name_match = re.search(
        r"Candidate[\s']*s?\s*Name\s*:?\s*([A-Za-z\s]+?)(?:\n|TC Name|Test|Application|Roll)",
        text,
        re.IGNORECASE
    )
    ...
    if name_match:
        ...
        name = re.sub(r"\s+", " ", name)  # Clean newlines/spaces
        details["name"] = name
